...
describe('Agent', () => {
  ...
  it('provides a proxy factory by default', () => {
    ...
    expect(() => Agent.remote({ address: 'test@fabric', fabric })).not.toThrow();
  });
  ...
  it('runMany aggregates results and exceptions', async () => {
    ...
    registerAgentProxyFactory({
      remoteByAddress: jest.fn((address: AddressLike, _options: { fabric: FabricLike }) => {
  ...
  it('broadcast maps addresses to targets', async () => {
    ...
    const results = await Agent.broadcast(['one@fabric', 'two@fabric'], payload, {
      fabric,
    });
  ...
  it('creates handler-backed agents', async () => {
    ...
    await expect(agent.getTaskStatus({ id: 'task-1' } as TaskQueryParams)).rejects.toThrow(
      /does not support operation 'getTaskStatus'/
    );
  });
